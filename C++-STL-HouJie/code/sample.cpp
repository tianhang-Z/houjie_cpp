...
void test_misc() {
  ...
  cout << size_t(-1)
       << endl;  // 4294967295        // 18446744073709551615 64位 (2^64-1)
  cout << "this machine supports max long num is: " << size_t(-1) / sizeof(long)
       << endl;  // 1073741823
  cout << "this machine supports max string num is: "
       << size_t(-1) / sizeof(string) << endl;  // 1073741823
  cout << "this machine supports max _List_node<string> num is: "
       << size_t(-1) / sizeof(_List_node<string>) << endl;  // 357913941
  cout << "this machine supports max _Fwd_list_node<string> num is: "
       << size_t(-1) / sizeof(_Fwd_list_node<string>) << endl;  // 536870911
  cout << "RAND_MAX= " << RAND_MAX
       << endl;  // 32767(2^15-1)  //2147483647 (2^32-1)
  ...
  bool test_MyString_hash = false;
  if (test_MyString_hash) {
...
void output_static_data(const T& myStr) {
  ...
  cout << " CCtor=" << T::CCtor << " MCtor=" << T::MCtor
       << " CAsgn=" << T::CAsgn << " MAsgn=" << T::MAsgn << " Dtor=" << T::Dtor
       << " Ctor=" << T::Ctor << " DCtor=" << T::DCtor << endl;
}
...
void test_moveable(M c1, NM c2, long& value) {
  ...
  cout << "construction, micro-seconds : " << (clock() - timeStart) << endl;
  cout << "size()= " << c1.size() << endl;
  output_static_data(*(c1.begin()));

  timeStart = clock();
  M c11(c1);
  cout << "copy, micro-seconds : " << (clock() - timeStart) << endl;

  timeStart = clock();
  M c12(std::move(c1));
  cout << "move copy, micro-seconds : " << (clock() - timeStart) << endl;

  timeStart = clock();
  c11.swap(c12);
  cout << "swap, micro-seconds : " << (clock() - timeStart) << endl;
...
void test_array() {
  ...
  cout << "micro-seconds : " << (timeEnd - timeStart) << endl;  //
  ...
  cout << "arr[0] after sort " << c[0] << endl;
  long target = get_a_target_long();

  long* pItem =
      (long*)::bsearch(&target, (c.data()), ASIZE, sizeof(long), compareLongs);
  cout << "qsort()+bsearch(), micro-seconds : " << (clock() - timeStart)
       << endl;  //
  if (pItem != NULL)
...
void test_vector(long& value) {
  ...
  for (long i = 0; i < value; ++i) {
    try {
      snprintf(buf, 10, "%d", rand());  //将字符格式化之后写到buf
      c.push_back(string(buf));
      int now_c_capacity = c.capacity();
      if (now_c_capacity != c_capacity) {
        cout << "capacity change from " << c_capacity << "to " << now_c_capacity
             << endl;

        c_capacity = now_c_capacity;
      }
    } catch (exception& p) {
  ...
  cout << "vector last value " << c[value - 1] << endl;

  string target = get_a_target_string();
  {
    timeStart = clock();
    auto pItem = find(c.begin(), c.end(), target);
    cout << "std::find(), micro-seconds : " << (clock() - timeStart) << endl;
...

  {
    timeStart = clock();
    sort(c.begin(), c.end());
    cout << "sort(), micro-seconds : " << (clock() - timeStart) << endl;

    timeStart = clock();
    string* pItem = (string*)::bsearch(&target, (c.data()), c.size(),
                                       sizeof(string), compareStrings);
    cout << "bsearch(), micro-seconds : " << (clock() - timeStart) << endl;
...
void test_queue(long& value) {
...

  {
    ...
    cout << "queue.front()= " << c.front() << endl;
    cout << "queue.back()= " << c.back() << endl;
    //! c.pop();  //[Error] 'class std::vector<std::basic_string<char> >' has no
...
void test_list_with_special_allocator() {
  ...
  cout << "a lot of push_back(), micro-seconds : " << (clock() - timeStart)
       << endl;
...
void test_components_sizeof() {
  ...
  cout << "sizeof(unordered_set<int>)= " << sizeof(unordered_set<int>)
       << endl;  // 28
  cout << "sizeof(unordered_map<int,int>)= " << sizeof(unordered_map<int, int>)
       << endl;  // 28
  cout << "sizeof(unordered_multiset<int>)= " << sizeof(unordered_multiset<int>)
       << endl;  // 28
  cout << "sizeof(unordered_multimap<int,int>)= "
       << sizeof(unordered_multimap<int, int>) << endl;  // 28
  cout << "sizeof(_Rb_tree<...>)= "
       << sizeof(_Rb_tree<int, int, _Identity<int>, less<int>>) << endl;  // 24
  ...
  //! << endl;	//[Error] 'iterator' is not a member of 'std::stack<int>' cout
  ...
  cout << "sizeof(set<int>::iterator)= " << sizeof(set<int>::iterator)
       << endl;  // 4
  cout << "sizeof(map<int,int>::iterator)= " << sizeof(map<int, int>::iterator)
       << endl;  // 4
  cout << "sizeof(multiset<int>::iterator)= " << sizeof(multiset<int>::iterator)
       << endl;  // 4
  ...
  cout << "sizeof(__gnu_cxx::malloc_allocator<string>)= "
       << sizeof(__gnu_cxx::malloc_allocator<string>) << endl;  // 1 (理論值 0)
  ...
  cout << "sizeof(__gnu_cxx::bitmap_allocator<string>)= "
       << sizeof(__gnu_cxx::bitmap_allocator<string>) << endl;  // 1 (理論值 0)
